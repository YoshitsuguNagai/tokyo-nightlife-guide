...
## V5 追加機能：公式サイトからのインポート（2026-09-22）

### 概要
管理画面（Super Admin のみ）に「⇩ インポート」メニューが追加されました。
店舗の公式サイトURLを入力すると、店舗情報・CAST一覧・各CAST詳細を自動解析し、
TNG の既存フィールドへ自動マッピングして**下書き**として登録できます。

### 使い方
1. 管理画面 → 「⇩ インポート」（または 店舗一覧の「公式サイトから店舗をインポート」）
2. 公式サイトURL を入力 →「情報を取得」（最大30秒）
3. 取得結果が ✓取得成功 / △要確認 / －情報なし のアイコン付きで表示されるので全項目を目視確認・修正
4. 使用する店舗写真を選択（画像は TNG 側へ再保存されます。直リンクではありません）
5. CAST を選択 →「選択したキャストの詳細を取得」→ 各CASTの項目と写真を確認
6. 「店舗＋キャストを下書き登録」
7. 店舗・キャスト一覧で内容を最終確認し、問題なければステータスを「公開」に変更

### 設計上の注意（仕様）
- インポートは常に status='draft'（下書き）。公開サイトには表示されません
- 店舗アカウント・キャストアカウントは自動発行されません（ユーザー管理から個別発行）
- 画像は外部URLを直リンクせず、Cloudinary（設定時）または /uploads へ再保存
- 同じURLで再インポートすると既存店舗を検出し、項目の差分（旧/新）と
  「公式サイトで確認できないキャスト」を表示します（TNG側は自動削除されません）
- 解析精度は公式サイトの構造に依存します。必ず人間が確認してから公開してください

### 追加ファイル
- importer.js … 取得・解析・再保存API（/api/admin/import/fetch, /cast, /rehost）
- public/admin.html … インポート確認UI
