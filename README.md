...
## 13. V4 最終版 追記（2026-09-09）

### データ永続化（重要）
- `DATABASE_PATH` 環境変数でDBの保存先を変更できます。Render では「Disks」→ Mount Path 例 `/var/data` を追加し、環境変数 `DATABASE_PATH=/var/data/data.sqlite` を設定すると再デプロイ後もデータが保持されます
- 画像は Cloudinary（§3）を必ず設定してください

### 今回の修正
- 日時を日本時間(JST)に統一（DB保存を datetime('now','+9 hours') へ）
- DMに種別を追加（店舗チャット/キャストチャット/問い合わせ/予約希望）＋ 予約は希望日時・人数を保存
- 口コミ公式返信の表示名を店舗名/キャスト名に
- お気に入りの状態表示（トースト文言・塗りつぶし）＋マイページを店舗/キャストのカード表示に
- クーポン詳細モーダル（期限切れ表示）＋マイクーポンをクリック可能に
- イベント日時・クーポン期限を date/time ピッカー化（終了日・利用開始日を追加）
- キャスト検索チップの選択同期不具合を修正、検索項目をJP/EN/中文で表示
- 保存失敗時はエラー表示（成功トーストはAPI成功時のみ）


---

## 14. V4 最終修正版（2026-09-09）

### 管理画面へのログイン（3権限とも同じURL）
- 管理画面URL: `/admin.html`（例: https://your-app.onrender.com/admin.html）
- ログインは共通画面で行い、権限（Super Admin / Club Admin / Cast）を自動判定してメニューを振り分けます
- Club Admin / Cast アカウントは、Super Admin が管理画面「ユーザー」→「新規追加」で作成します
  - 権限「店舗管理者」→「対象店舗」を選択 / 権限「キャスト」→「対象キャスト」を選択
  - 初期パスワード（8文字以上）を設定、または空欄で招待リンクを発行して本人に共有

### 今回の修正
- チャット/予約送信時の unauthorized: 古いトークンを自動破棄し、未ログイン時は「チャットを利用するにはログインしてください」を3言語で表示
- 予約時間の選択肢を 20:00〜 に変更、予約成功時は「予約希望を送信しました」
- キャストの TikTok / X URL が保存・公開ページへ反映されない不具合を修正（SVGブランドアイコン、未登録は非表示）
- Google Maps: embed/iframe URL を外部リンクに使わない構造に完全分離
- キャスト検索: 「この条件で検索」ボタンを削除（リアルタイム検索のみ）＋「条件をクリア」で全解除。内部名（f_style_type等）は「系統/Type/类型」「スタイル/Style/身材」と表示
- お気に入り: 追加/解除のトースト文言、ハートの塗りつぶし状態、再読み込み後の状態復元
- 管理画面ログインのパスワード欄に目アイコン（SVG）追加、初期管理者情報の画面表示を除去
